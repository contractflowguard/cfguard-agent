...
* `/report <project_name> [table|html|json]` — отчёт по проекту  
  *table* → прикрепляется как `.txt`‑файл с метриками в начале;  
  *html*  → прикрепляется как `.html`‑файл с футером-резюме;  
  *json*  → прикрепляется как `.json`‑файл с расчетными полями (дельта, статус) по каждой задаче.  
  ⓘ В начале отчёта выводятся ключевые метрики: сколько задач не начаты, сколько минут просрочено.
