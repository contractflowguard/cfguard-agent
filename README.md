...
* `/report <project_name> [table|html|json]` — отчёт по проекту  
  ...
  *html*  → `.html`‑файл с футером-резюме, SVG-графиком по статусам, summary-метриками.  
...
```bash
# В начале отчёта будет summary-блок с ключевыми показателями (⚠️ не начаты, ⏱ просрочено)
...
- `parent_id` — ID родительской задачи (если есть, для построения иерархии)
- `is_group` — является ли задачей-группой (TRUE/FALSE)
- `level` — уровень вложенности задачи (0 для корневых)
